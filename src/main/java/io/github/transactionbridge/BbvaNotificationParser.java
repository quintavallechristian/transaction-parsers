...
public final class BbvaNotificationParser implements NotificationParser {
    private static final Pattern PAYMENT = Pattern.compile(
            "Il pagamento di ([0-9][0-9.,]*) EUR in data (.+?) effettuato con la tua carta [0-9]{4} è stato accettato\\.?",
    ...
    @Override public Transaction parse(long occurredAt, String rawText) {
        ...
        try {
            ...
            return amount.signum() > 0 && !merchant.isEmpty()
                    ? new Transaction(occurredAt, amount, "EUR", merchant, text, "bbva-notification")
                    : null;
